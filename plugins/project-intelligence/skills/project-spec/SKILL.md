...
For requirement-level implementation, create or register `.project-intel/requirements/<id>-<title>/requirement.md` and keep its numbered acceptance criteria identical to `manifest.acceptanceCriteria`. Legacy `<id>/` archives remain readable. Do not create `.project-intel/specs/`.
...
project-intel requirement add --requirement-id "<id>" --type requirement \
  --path .project-intel/requirements/<id>-<title>/requirement.md
```
