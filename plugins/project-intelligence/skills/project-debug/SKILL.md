...
7. Each evidence path must resolve inside the repository and identify the file or `path#symbol` that supports the root-cause claim; repeat `--evidence` when more than one source location is necessary. A Bug cannot pass `requirement ready` without a current diagnosis. After `project-design` and optional planning, invoke `project-test` before ready to confirm the evidence mode, explicit AC mapping, and mandatory test-document action: register a valid supplied report or generate one. After ready succeeds and `project-task` runs `requirement begin`, add a failing regression test or minimal reproduction before implementing the fix, then record the expected RED output.
...
When the bug fix changes code, use exactly this pre-edit lifecycle: `project-intake --ticket-kind bug` → `project-spec` → `project-debug` → `project-design` → optional `project-plan` → `project-test` contract → `requirement ready`. Do not route back from design to spec unless source evidence exposes a real requirement contradiction that the user must resolve. Then run `project-intel requirement begin --requirement-id "<id>"` before editing or executing tests, and pass that ID through RED, GREEN, review, finish, and maintain.
