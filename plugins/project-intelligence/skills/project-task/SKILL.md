...
3. Identify related modules, components, Hooks, APIs, services, routes, and standards.
4. Prefer existing project abstractions before creating new ones.
5. Treat redundancy findings as `candidate` unless a rule has been promoted to `hard`.
6. Use GitNexus for symbol-level calls, impact, and change risk when available.
7. Use Understand-Anything for architecture, module, and domain-flow context when available.
8. Before the first code edit, run impact/reuse analysis with GitNexus impact/explore tools when available; otherwise use `.project-intel`, `project-intel query "<symbol-or-feature>"`, or `project-intel lifecycle --task "<requirement>"`. `lifecycle` prints by default and includes track/readiness; use `--write` only when the user explicitly wants a persistent task-impact report.
9. Decide execution mode:
...
10. If implementation reveals a bug, error, test failure, or regression, switch to `project-debug` before proposing fixes.
11. Use `.project-intel`, GitNexus, Understand-Anything, and direct source reads as the task context sources.
